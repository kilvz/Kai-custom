...
### Fixes
- Align all MCP tool names with alt-memory v4.5.0 — 12/13 mismatched names corrected (add_entity, get_entity, delete_entity, search, export_collection, import_entities, kg_add, kg_query, kg_invalidate, record_write, record_read, set_persona)
- Fix migration loop in AltMemoryLifecycleManager to use correct add_entity schema (entity_id/realm/domain/metadata)

### Features
- Wire persona linking between app and alt-memory: built-in personas (kai, alt) sync on startup, remote personas merge into local list, save/delete operations push to alt-memory via set_persona/delete_persona

### Improvements
- Add architecture documentation (docs/architecture/memory-beta.md, persona-system-prompt-plan, upstream-map)

## v3.1.2 — 2026-06-01

### Fixes
- Resolve compilation errors from upstream merge
- Fix ktlint violations across codebase

## v3.1.0 — 2026-06-01

### Features
- AutoMemoryLearner: inline memory extraction every 5 exchange pairs, stores user facts/preferences/entities
- HeartbeatMemoryExtractor: post-heartbeat behavior pattern extraction, stores as protected memories
- Two-tier memory storage: protected (behavior learnings, hidden from UI, cannot be deleted) vs unprotected (user facts/preferences, deletable)
- Soul split: soul_user (editable in Settings) + soul_auto (auto-generated by heartbeat), combined in getSoulText()
- Persona system: PersonaConfig/PersonaManager with id, name, description, prompt style, heartbeat style; Settings UI with persona name field
- Memory toggle: disables memory/KG/diary tools, extraction, and prompt dump when off
- Protected toggle in MemoryManagementSheet: "Show protected" switch reveals behavior learnings (cannot delete)

### Improvements
- Trim ChatSystemPromptBuilder: removed 6 sections (Tool Use, When to Act, Memory System, Automation, Structured Learning, email policy)
- New prompt order: Soul → Language → Honesty → What I Know About You (1KB cap) → Email → Tasks → Heartbeat → Context → kai-ui
- HeartbeatPromptBuilder includes Learned Patterns section from behavior memories
- Version aligned: 3.1.0

## v3.0.0 — 2026-05-31

### Features
- Local memory/KG/diary engine with TFLite text embedding and vector search
- SQLite-to-alt-memory migration on first startup
- Memory management UI
- Dimension sync between local SQLite and alt-memory MCP server

## v2.0.17 — 2026-05-31

### Fixes
- WakeWordService crash: move startForeground() before anti-flap check
- Duplicate apt sources in Ubuntu sandbox
- Port upstream OpenAPI fix (getInstanceBaseUrl)

### Features
- Distro-aware Packages tab in sandbox
- Sandbox skills support
- Port cross-platform tools to iOS/Desktop/WasmJS + fix multiplatform compilation
