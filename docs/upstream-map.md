...
| Upstream version at fork | `v2.6.3` (code 107) | ΓÇö |
| Our current version | ΓÇö | `v2.1.0` (code 132) on `main`; `v3.1.2` (code 134) on `beta` |
| Package | `com.inspiredandroid.kai` | `com.kai.custom` |
| Git tag for fork point | `v2.6.3` | `2b7d400` |
| Upstream HEAD (latest) | ~`bc7d622` (v2.7.0) | ΓÇö |

### Version Mapping

Upstream tags ΓåÆ Fork equivalents (both branches diverge after fork point).

| Upstream Tag | Upstream Version | Fork `main` | Fork `beta` |
|---|---|---|---|
| `v2.6.0` | 2.6.0 (code 105) | ΓÇö | ΓÇö |
| `v2.6.1` | 2.6.1 (code 106) | ΓÇö | ΓÇö |
| ΓÇö | ΓÇö | 2.0.0ΓÇô2.1.0 (code 124ΓÇô132) | 3.0.0ΓÇô3.1.2 (code 128ΓÇô134) |
...
| `App.kt` | `App.kt` | **Modified** ΓÇö +SandboxAwareUriHandler |
| `AppModule.kt` | `AppModule.kt` | **Modified** ΓÇö +SkillManager, AutoMemoryLearner, HeartbeatMemoryExtractor, DimensionStore |
| `data/AppSettings.kt` | `data/AppSettings.kt` | **Modified** ΓÇö soul split (soul_user+soul_auto), persona system, memory toggle |
| `data/AppSettingsImportExport.kt` | `data/AppSettingsImportExport.kt` | **Modified** ΓÇö exports only soul_user |
...
| `data/ChatSystemPromptBuilder.kt` | `data/ChatSystemPromptBuilder.kt` | **Modified** ΓÇö trimmed (removed 6 sections), memory budget cap 1024 chars |
| `data/DataRepository.kt` | `data/DataRepository.kt` | **Modified** ΓÇö +skill methods, +schemaResetMessage, +protected memory, +soul split, +persona, +getRecentExchanges |
| `data/MemoryStore.kt` | `data/MemoryStore.kt` | **Modified** ΓÇö +protected field, +storeProtected/getUserMemories/getBehaviorMemories, +schemaResetMessage |
| `data/RemoteDataRepository.kt` | `data/RemoteDataRepository.kt` | **Modified** ΓÇö +skill wiring, soul split, AutoMemoryLearner, persona, getRecentExchanges |
| `data/ToolExecutor.kt` | `data/ToolExecutor.kt` | **Modified** ΓÇö +caching (cachedTools/cachedToolDefs), +invalidateCache |
| `data/TaskScheduler.kt` | `data/TaskScheduler.kt` | **Modified** ΓÇö +HeartbeatMemoryExtractor, +memoryStore param |
| `data/HeartbeatManager.kt` | `data/HeartbeatManager.kt` | **Modified** ΓÇö +learnedPatterns, builds with behavior memories |
| `data/HeartbeatPromptBuilder.kt` | `data/HeartbeatPromptBuilder.kt` | **Modified** ΓÇö +Learned Patterns section |
| `data/ModelCatalog.kt` | `data/ModelCatalog.kt` | **Modified** ΓÇö +hunyuan-hy3-preview, updated Elo scores 2026-05-31 |
...
| `ui/chat/ChatActions.kt` | `ui/chat/ChatActions.kt` | **Modified** ΓÇö +skill actions |
| `ui/chat/ChatUiState.kt` | `ui/chat/ChatUiState.kt` | **Modified** ΓÇö +installedSkills, +schemaResetMessage |
| `ui/chat/ChatViewModel.kt` | `ui/chat/ChatViewModel.kt` | **Modified** ΓÇö +skill invocation, +formatException extracted to Util.kt |
| `ui/chat/ChatScreen.kt` | `ui/chat/ChatScreen.kt` | **Modified** ΓÇö +installedSkills prop to QuestionInput |
| `ui/chat/composables/QuestionInput.kt` | `ui/chat/composables/QuestionInput.kt` | **Modified** ΓÇö +slash autocomplete, +detectSlashQuery |
| `ui/settings/SettingsUiState.kt` | `ui/settings/SettingsUiState.kt` | **Modified** ΓÇö +6 skill fields, PendingDeletion.Skill, +schemaResetMessage |
| `ui/settings/SettingsViewModel.kt` | `ui/settings/SettingsViewModel.kt` | **Modified** ΓÇö +skill management, +schemaResetMessage |
| `ui/settings/SettingsScreen.kt` | `ui/settings/SettingsScreen.kt` | **Modified** ΓÇö +SkillsSection, +schemaResetMessage AlertDialog |
| `ui/settings/SettingsActions.kt` | `ui/settings/SettingsActions.kt` | **Modified** ΓÇö +4 skill actions |
| `ui/settings/ToolsSettings.kt` | `ui/settings/ToolsSettings.kt` | **Modified** ΓÇö +SkillsSection |
...
| `Platform.android.kt` | `Platform.android.kt` | **Modified** ΓÇö +memory toggle gating, +wake word support |
...
|---|---|---|
| `testutil/FakeDataRepository.kt` | `testutil/FakeDataRepository.kt` | **Modified** ΓÇö +skill stubs, +protected memory stubs, +soul split stubs, +persona stubs, +schemaResetMessage |
...
|---|---|
| `ui/chat/ChatViewModelSkillTest.kt` | Skipped ΓÇö architecture mismatch |
| `ui/chat/DetectSlashQueryTest.kt` | Skipped ΓÇö function inlined |
...
|---|---|---|
| `values/strings.xml` | Yes | **Modified** ΓÇö +22 skill strings, +snackbar_skill_removed |
| `values-*/strings.xml` (54 locale files) | Yes | **Modified** ΓÇö +snackbar_skill_removed |
...
| **Tools ΓÇö unique** | Basic set | +ADB, SSH, Shizuku, TTS, OpenCode integration, phone tools, process management |
...
Upstream: (no equivalent ΓÇö fork-only)
```
