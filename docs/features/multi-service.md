...
A built-in service that requires no API key. Free is never shown in the service picker ΓÇö it is used as:
...
8. While the chain is being walked, the thinking indicator shows per-attempt status ΓÇö the name of the service currently being tried, or the reason the previous one failed before moving on ΓÇö so silent fallbacks are visible to the user
9. Entries whose context window can't fit the current chat history are skipped during the walk
10. On-device (Local Model) failures are not silently absorbed ΓÇö they short-circuit the fallback chain so the user sees the actual error rather than being quietly bumped to a cloud service
11. Certain non-retryable errors (notably Anthropic's "insufficient credits" and quota-exhausted responses from OpenAI-compatible providers) bypass both the per-service retry loop and the fallback chain ΓÇö the error surfaces immediately
...
- **Title** (top left) ΓÇö a human-readable display name from the curated catalog or the provider's API; falls back to the raw model id only when no display name is available
- **Arena score** (top right) ΓÇö LMArena Elo rating as colored text, gradient from green (>= 1400) through lime/yellow to orange (< 1250)
- **Detail line** (below the title) ΓÇö release date, parameter count, and context window joined into a single muted line separated by ` ┬╖ ` (e.g. `Mar 2025 ┬╖ 70B ┬╖ 200K ctx`); any missing field is simply omitted from the line
