...
Kai can **read** incoming SMS messages and **draft** outgoing SMS for the user to review and send. Read and Send are independent user opt-ins: the user can let Kai read SMS without also letting it send, and vice versa. Sending is always gated by an explicit user tap in a review banner â€” the AI never sends directly.
...
The gate is manifest-based: `androidApp/src/foss/AndroidManifest.xml` declares both `READ_SMS` and `SEND_SMS`. At runtime the app queries `PackageManager.getPackageInfo(â€¦, GET_PERMISSIONS).requestedPermissions` to decide whether to show the feature.

## Scope

- **Read**: list / read / search inbox SMS.
- **Send** (including replies): AI drafts a message; user must tap Send in the review banner before anything is actually transmitted. The AI cannot bypass the banner â€” the `send_sms` / `reply_sms` tools only *stage* a draft; the actual `SmsManager.sendTextMessage` call is initiated by the tap handler in the UI.
...
1. In **Settings â†’ Agent â†’ SMS â†’ "Read incoming SMS"**, the user flips the toggle on.
...
5. On the next heartbeat, the queue snapshot is included in the prompt under `## New SMS`. After the heartbeat run, exactly that snapshot is removed from the queue â€” messages that arrived during the call survive to the next heartbeat.
...
- `lastSeenId` â€” the largest inbox `_id` that has been pushed into pending. SMS rowids are globally monotonic in the system SMS database, so a single high-water mark is sufficient even with multiple SIMs.
- `lastSyncEpochMs` â€” last successful poll.
- `lastAttemptEpochMs` â€” last attempted poll (success or failure).
- `unreadCount` â€” number of inbox rows fetched on the last poll that were not marked read by the system UI.
- `lastError` â€” human-readable error from the last failed attempt, or null.
...
- Stores `id`, `address` (phone number), `date`, and a truncated `preview` (â‰¤200 chars). The full body is fetched on demand by the `read_sms` tool â€” this keeps the encrypted-prefs JSON small and avoids duplicating message bodies the system already stores.
...
**Send tools** (gated on `isSmsSupported && isSmsSendEnabled && hasSendPermission`). Both *stage* drafts â€” neither sends directly:

| Tool | Purpose |
|---|---|
| `send_sms` | Draft an outgoing SMS to a phone number. Returns a `draft_id`. |
| `reply_sms` | Draft a reply to a received SMS by its `id` â€” sender looked up from the inbox. Returns a `draft_id`. |
...
The SMS section appears in **Settings â†’ Agent** only when `isSmsSupported` is true. It contains two independent sub-toggles in one card:

**Read incoming SMS** (requires `READ_SMS`):

- Toggle â€” enabling it triggers the runtime permission request. If permission is denied, the toggle stays off.
- "Grant permission" button â€” shown when the toggle is on but permission has not been granted (or was revoked).
- Poll interval slider â€” Never / 5m / 15m / 30m / 60m.
- Queued count â€” number of SMS sitting in the pending queue waiting for the next heartbeat.
- Last poll status â€” "Last successful poll 5m ago" or "Last poll failed 1h ago" (with the error surfaced in the error color).
- Refresh icon â€” forces a one-shot poll (shows a spinner while polling).

**Send SMS on your behalf** (requires `SEND_SMS`):

- Toggle â€” enabling it triggers the runtime permission request. Gates the `send_sms` / `reply_sms` tool registration.
- "Grant permission" button â€” shown when the toggle is on but permission has not been granted (or was revoked).
