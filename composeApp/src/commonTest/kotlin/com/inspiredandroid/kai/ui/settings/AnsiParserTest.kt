...
class AnsiParserTest {
    ...
    fun `256 color foreground via 38 5 sets exact color`() {
        // index 196 is the bright red corner of the 6x6x6 cube: r=5, g=0, b=0 â†’ (255, 0, 0)
    ...
    fun `256 color grayscale range maps to gray colors`() {
        // index 232 â†’ gray = (232-232)*10 + 8 = 8
    ...
    fun `non-SGR CSI sequences are stripped`() {
        // Cursor up (ESC[A), erase line (ESC[2K) â€” should be removed but text preserved
    ...
    fun `unknown escape after ESC is consumed`() {
        // ESC followed by neither [ nor ] â€” both bytes are skipped
        val input = "A${esc}xB"
        val result = parseAnsiToAnnotatedString(input, defaultColor)
