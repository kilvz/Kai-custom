...
/** Tests for [MathParser] â€” LaTeX fragments â†’ [MathAtom] tree. */
class MathAtomParserTest {
    ...
    fun `greek letter command resolves to Unicode VARIABLE sym`() {
        assertEquals(Sym("Î±", SymKind.VARIABLE), parse("\\alpha"))
        assertEquals(Sym("Ï€", SymKind.VARIABLE), parse("\\pi"))
        assertEquals(Sym("Î£", SymKind.ORDINARY), parse("\\Sigma"))
    }
    ...
    fun `superscript with braces groups the whole expression`() {
        ...
        assertEquals(Sym("Ï€", SymKind.VARIABLE), sup.atoms[1])
    }
    ...
    fun `arrow relation`() {
        ...
        val atoms = (atom as Group).atoms
        assertEquals(Sym("âŸ¶", SymKind.REL_OP), atoms[1])
    }
    ...
    fun `primes collapse into single sym`() {
        ...
        assertEquals("â€²â€²", sup.text)
    }

    // â”€â”€ New-construct coverage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    @Test
    fun `implies and iff map to long arrows`() {
        assertEquals(Sym("âŸ¹", SymKind.REL_OP), parse("\\implies"))
        assertEquals(Sym("âŸº", SymKind.REL_OP), parse("\\iff"))
    }
