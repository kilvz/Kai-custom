    ...
    fun installPackages() {
        ...
        currentJob = scope.launch {
            try {
                ...
                // Install packages.
                // Alpine: per-package sequential (apk is fast, better error isolation).
                // Ubuntu: download-first (parallel fetch is faster than 10 sequential),
                //         then install from cache, with dpkg recovery and per-package
                //         fallback if all else fails.
                if (distro == "ubuntu") {
                    val downloadTimeout = 300L

                    // Phase 1 — download-only: safe, never touches dpkg, parallel fetch
                    ensureActive()
                    ...
                    for (attempt in 1..2) {
                        ...
                        for (mirror in mirrors) {
                            ...
                            val updated = executor.execute("apt-get update", timeoutSeconds = updateTimeout)
                            if (updated["success"] as? Boolean != true) continue
                    ...
                    ensureActive()
                    if (!downloaded) {
                        android.util.Log.w("LinuxSandbox", "Download phase failed — proceeding with direct install")
                    }
