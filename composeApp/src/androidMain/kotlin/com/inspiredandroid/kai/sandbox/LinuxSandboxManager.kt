...
class LinuxSandboxManager(
    ...
    // down. Live during the app process only â€” not persisted.
        ...
        currentJob = scope.launch {
            ...
            val result = executor.execute(updateCmd, timeoutSeconds = 180)
                ...
                if (!updated) {
                    ...
                    _state.value = SandboxState.Error("$updateCmd failed â€” check device network connectivity")
                    return@launch
                }
    ...
    fun getDiskUsageMB(): Long {
        ...
        // AssertionError if a child entry transitions from directoryâ†’non-directory
