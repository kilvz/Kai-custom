...
object ShellCommandTool : Tool {
    ...
    override val schema: ToolSchema
        get() = ToolSchema(
            ...
            parameters = mapOf(
                ...
                "working_dir" to ParameterSchema("string", "If set, run the command starting in this directory (cd <dir> && <command>). The cd persists for subsequent calls â€” same as if the user had run cd themselves.", false),
    ...
    private fun buildDescription(): String = buildString {
        ...
        appendLine()
        appendLine("IMPORTANT â€” Android system commands (pm, am, dumpsys, settings, wm, input, device_config, content, appops, cmd, and anything under /system, /vendor, /apex) do NOT exist in this sandbox. It is NOT the Android shell. For ADB-level system commands, use the separate run_adb tool instead â€” that tool goes through Shizuku and can access all Android system services.")
        appendLine()
        ...
        appendLine("- Default timeout: 30s, max: 60s. Long-running interactive commands (e.g. ssh sessions held across messages) work because the shell is persistent â€” but a SINGLE call still hits the timeout if it doesn't return.")
        appendLine("- Fullscreen TUIs (top, htop, vim, less, nano, anything ncurses) WILL NOT WORK â€” the sandbox has no PTY. Use non-interactive variants: \"top -bn1\" for a one-shot snapshot, \"ps aux\" for processes, redirect editor output, etc.")
