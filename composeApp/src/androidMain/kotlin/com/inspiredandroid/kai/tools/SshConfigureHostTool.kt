...
object SshConfigureHostTool : Tool {
    ...
    override val schema: ToolSchema
        ...
        parameters = mapOf(
            ...
            "identity_file" to ParameterSchema(
                "string",
                "Path to the private key inside the sandbox. Relative names resolve under ~/.ssh (e.g. 'my-vps_id' â†’ ~/.ssh/my-vps_id); absolute paths and ~-paths pass through. The file is not created by this tool.",
                false,
            ),
    ...
    private fun buildDescription(): String = buildString {
        ...
        appendLine("- Defaults block at the top of the config on first use: ServerAliveInterval + ServerAliveCountMax (keep idle TCP connections alive through NAT) and StrictHostKeyChecking=accept-new (auto-accept new host keys into ~/.ssh/known_hosts on first connect, but still reject changed keys â€” sane TOFU without an interactive prompt this shell can't answer).")
        appendLine("- Optionally appends a line to ~/.ssh/known_hosts to skip the first-connect TOFU step entirely.")
        appendLine()
        appendLine("This tool does NOT create or upload private keys. To make a key usable, the user must place it under ~/.ssh in the sandbox separately. Be aware that any key text passed through chat (including via execute_shell_command's `cat > ~/.ssh/id_x <<EOF ...`) goes to the model provider in cleartext â€” ask the user before doing that.")
        appendLine()
        ...
        appendLine()
        appendLine("Connection persistence (\"held connections\") is NOT available â€” openssh's ControlMaster multiplexing requires the link() syscall to create its control socket, and Android blocks link() for app processes regardless of file ownership. Each ssh call does a full handshake. Don't fight this; don't try to seed your own ControlPath.")
        appendLine()
        ...
        append("Auth, port, identity all come from the config block â€” no flags needed. ALWAYS invoke by the alias, never `user@hostname`; bypassing the alias bypasses every setting this tool just wrote.")
    }
