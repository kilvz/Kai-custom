 ...
 *
 * By default this test is **skipped** â€” it only runs when `KAI_INTEGRATION=1` is set
 ...
 * ```
 *
 * Any subset of provider keys can be supplied â€” the test auto-detects which ones are
 ...
 *
 * Additional knobs:
 * - `KAI_REPORT_DIR` â€” where to write the report (default: `build/reports/kaiui-integration`)
 * - `KAI_MIN_SUCCESS_RATE` â€” 0.0-1.0, fail the test below this rate (default: 0.0, never fail)
 * - `KAI_REPLAY_STRICT` â€” set to `1` to make the replay test fail when any saved response
 *   no longer parses cleanly. Default off (report-only).
 *
 * ## What it produces
 *
 * Under the report directory:
 * - `report.md` â€” overall summary table plus per-provider/per-prompt status
 * - `<provider>/<slug>.raw.txt` â€” raw assistant response for every prompt
 * - `<provider>/<slug>.error.txt` â€” only for prompts where the parser produced
 *   `ErrorSegment` or no `UiSegment` at all; contains raw JSON plus parser diagnostics
 *
 * The `.raw.txt` files are pure LLM output with no headers â€” safe to feed back
...
class KaiUiValidationTest {
    ...
    fun `validate kai-ui output across providers`() {
        if (System.getenv("KAI_INTEGRATION") != "1") {
            println("[KaiUiValidationTest] Skipped â€” set KAI_INTEGRATION=1 to run.")
            return
        }

        val providers = discoverProviders()
        if (providers.isEmpty()) {
            println("[KaiUiValidationTest] Skipped â€” no provider API keys found in env.")
     ...
     * API keys â€” use this while iterating on parser fixes or prompt tweaks.
     *
    ...
    fun `replay saved responses`() {
        ...
        if (!reportDir.isDirectory) {
            println("[KaiUiValidationTest] Replay skipped â€” no report directory at ${reportDir.absolutePath}")
            return
        }
        ...
        if (rawFiles.isEmpty()) {
            println("[KaiUiValidationTest] Replay skipped â€” no .raw.txt files under ${reportDir.absolutePath}")
     ...
     * Keep in sync manually if the real prompt changes â€” there is no automated drift check.
     */
    private fun buildSystemPrompt(mode: TestPrompt.Mode): String = buildString {
        ...
        if (dynamicUiOnly) {
            ...
            append("- Use rows for groups of buttons or chips â€” rows wrap automatically\n")
    ...
    companion object {
        private val TEST_PROMPTS = listOf(
            ...
            TestPrompt(
                "nested-cards",
                "Nested layouts â€” card containing row containing buttons",
