 ...
 * - `training_data.jsonl` â€” 90% of examples
 * - `validation_data.jsonl` â€” 10% of examples
 *
 * ## Sources
 *
 * 1. **Golden examples** in `tools/finetuning/golden/` (`.md` files) â€” hand-crafted ideal responses
 * 2. **Saved integration test responses** in `build/reports/kaiui-integration/` â€” curated
...
class GenerateTrainingData {
    ...
    fun `generate training data`() {
        ...
        println("  Training:   ${training.size} examples â†’ ${File(outputDir, "training_data.jsonl").absolutePath}")
        println("  Validation: ${validation.size} examples â†’ ${File(outputDir, "validation_data.jsonl").absolutePath}")
    }
    ...
    private fun loadCuratedResponses(reportDir: File): List<TrainingExample> {
        ...
        // Map slug â†’ prompt from the standard test prompts
        val promptMap = mapOf(
