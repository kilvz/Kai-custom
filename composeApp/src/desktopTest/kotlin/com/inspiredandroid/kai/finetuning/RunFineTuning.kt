 ...
 * |------------------------|----------|-------------------|------------------------------------|
 * | MISTRAL_API_KEY        | Yes      | â€”                 | Your Mistral API key               |
...
class RunFineTuning {
    ...
    fun `run fine-tuning job`() = runBlocking {
        if (apiKey.isBlank()) {
            println("[RunFineTuning] Skipped â€” set MISTRAL_API_KEY to run.")
            return@runBlocking
        }
        ...
        if (finalJob.fine_tuned_model != null) {
            ...
            println("  Fine-tuned model: (not available â€” job may have failed)")
        ...
        while (true) {
            ...
            if (job.checkpoints.size > lastCheckpointCount) {
                ...
                for (cp in newCheckpoints) {
                    val trainLoss = cp.metrics?.train_loss?.let { "%.6f".format(it) } ?: "â€”"
                    val validLoss = cp.metrics?.valid_loss?.let { "%.6f".format(it) } ?: "â€”"
