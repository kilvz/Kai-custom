...
class DockerManager {
    ...
    suspend fun createContainer(
        ...
        bindMounts.forEach { (host, container) ->
            args.add("-v"); args.add("$host:$container")
        }
        portMappings.forEach { (host, container) ->
            args.add("-p"); args.add("$host:$container")
        }
        args.add(image)
        args.add("tail")
        args.add("-f")
    ...
    suspend fun execCommand(containerName: String, command: String): String {
        return runCommandWithOutput("docker", "exec", containerName, "sh", "-c", command)
    }
    ...
    suspend fun copyIn(containerName: String, hostPath: String, containerPath: String): Boolean {
        return runCommand("docker", "cp", hostPath, "$containerName:$containerPath")
    }

    suspend fun copyOut(containerName: String, containerPath: String, hostPath: String): Boolean {
        return runCommand("docker", "cp", "$containerName:$containerPath", hostPath)
    }

    suspend fun exportContainer(containerName: String, outputPath: String): Boolean {
        return runCommand("docker", "export", containerName, "-o", outputPath)
    }

    suspend fun importContainer(tarPath: String, imageName: String): Boolean {
        return runCommand("docker", "import", tarPath, imageName)
    }
    ...
    private suspend fun installDockerWindows(): Boolean {
        ...
        return try {
            runCommand("curl", "-L", "-o", installer.absolutePath,
                "https://desktop.docker.com/win/main/amd64/Docker%20Desktop%20Installer.exe")
            if (!installer.exists()) return false
            val proc = Runtime.getRuntime().exec(arrayOf(
                installer.absolutePath, "install", "--quiet"
            ))
    ...
    private suspend fun installDockerMac(): Boolean {
    ...
    private suspend fun runCommand(vararg args: String): Boolean {
        return try {
    ...
    private suspend fun runCommandWithOutput(vararg args: String): String {
        return try {
            val proc = Runtime.getRuntime().exec(args)
    ...
    private suspend fun containerInspect(name: String, format: String): String? {
        return try {
