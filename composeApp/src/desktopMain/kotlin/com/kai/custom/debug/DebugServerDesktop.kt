...
import com.kai.custom.data.EmailAccountDebugView
    ...
    fun start() {
        ...
        server = embeddedServer(CIO, port = 18500, host = "127.0.0.1") {
            ...
            routing {
                // ======================= HEALTH =======================
                get("/health") {
                    call.respondText(json.encodeToString(mapOf("status" to "ok", "token" to token)), ContentType.Application.Json)
                }
                ...
                get("/settings") {
                    ...
                    val personaName = dataRepository.getPersonaName()
                    ...
                    call.respondText(json.encodeToString(buildJsonObject {
                        ...
                        put("persona_name", JsonPrimitive(personaName))
                ...
                get("/email") {
                    ...
                    call.respondText(json.encodeToString(dataRepository.getEmailAccounts().map {
                        EmailAccountDebugView(id = it.id, email = it.email, displayName = it.displayName)
                    }), ContentType.Application.Json)
                }
                ...
                get("/sms/drafts") {
                    val err = auth(call) ?: return@get
                    call.respondText(json.encodeToString(emptyList<SmsDraftSummary>()), ContentType.Application.Json)
                }

                post("/sms/send/{draftId}") {
                    val err = auth(call) ?: return@post
                    call.respondText(json.encodeToString(buildJsonObject { put("success", JsonPrimitive(false)); put("error", JsonPrimitive("SMS not available on desktop")) }), ContentType.Application.Json)
                }
                ...
                // ======================= LOCAL INFERENCE (stub — desktop) =======================
                get("/local/models") {
                    val err = auth(call) ?: return@get
                    call.respondText(json.encodeToString(emptyList<LocalModelSummary>()), ContentType.Application.Json)
                }

                post("/local/download") {
                    val err = auth(call) ?: return@post
                    call.respondText(json.encodeToString(ErrorResponse("Local inference not available on desktop")), ContentType.Application.Json, HttpStatusCode.BadRequest)
                }
                ...
                // ======================= WAKE WORD (stub — desktop) =======================
                get("/wake-word") {
                    val err = auth(call) ?: return@get
                    call.respondText(json.encodeToString(WakeWordSettings(
                        enabled = false, phrase = "hey kai", mode = "GENERAL", template = "",
                    )), ContentType.Application.Json)
                }
                ...
                get("/whatsapp") {
                    ...
                    call.respondText(json.encodeToString(com.kai.custom.data.WhatsAppStatusResponse(
                        ...
                        pendingCount = WhatsAppStore(appSettings).getPending().size,
                    )), ContentType.Application.Json)
                }
                ...
                post("/whatsapp/install") {
                    ...
                    try {
                        val ok = withContext(Dispatchers.Default) {
                            sandboxController.installWhatsAppBridge()
                        }
                        if (ok) {
                ...
                post("/chat") {
                    ...
                    val result = withContext(Dispatchers.Default) {
                        try { dataRepository.askWithToolsVerbose(chatRequest.message) } catch (e: Exception) { com.kai.custom.data.AskWithToolsResult("Error: ${e.message}") }
                    }
                ...
                post("/import") {
                    ...
                    val sections = if (body.sections.isEmpty()) ImportSection.entries.toSet() else body.sections.map { ImportSection.valueOf(it.uppercase()) }.toSet()
                ...
                get("/admin/status") {
                    val err = auth(call) ?: return@get
                    call.respondText(json.encodeToString(mapOf("is_admin" to AdminManager.isAdmin())))
                }

                post("/admin/elevate") {
                    val err = auth(call) ?: return@post
                    call.respondText(json.encodeToString(mapOf("success" to AdminManager.relaunchAsAdmin())))
                }
