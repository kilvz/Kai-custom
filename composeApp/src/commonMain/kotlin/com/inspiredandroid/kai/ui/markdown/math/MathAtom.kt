...
enum class SymKind {
    /** Variable letters â€” rendered italic. */
    VARIABLE,

    /** Digits, punctuation, unit-like glyphs â€” rendered upright. */
    ORDINARY,

    /** Binary operators (+, -, Â·, Ã—) â€” upright with symmetric spacing. */
    BIN_OP,

    /** Relation operators (=, <, >, â‰¤, â‰ˆ, â†’) â€” upright with symmetric spacing. */
    REL_OP,

    /** Function names like sin, cos, log, lim â€” upright, not italic. */
    FUNCTION,
    ...
    /** Punctuation â€” upright, no extra spacing. */
    PUNCT,
}
...
/**
 * Big operator like âˆ‘, âˆ«, âˆ, â‹ƒ. In display mode [sub]/[sup] are typeset above/below the
...
/** `\left X ... \right Y` â€” brackets stretch to the height of [content]. */
@Immutable
...
/**
 * `\hat{x}`, `\bar{x}`, `\vec{v}`, `\tilde{y}`, `\dot{x}`, `\ddot{x}` â€” single-glyph accent
...
enum class MatrixAlign {
    /** All cells horizontally centered â€” default for pmatrix / bmatrix / matrix / vmatrix. */
    CENTERED,

    /** All cells left-aligned â€” used by `cases`. */
    LEFT,

    /** Odd columns right-aligned, even columns left-aligned â€” `aligned` / `align`. */
    ALIGN_RL,
}

enum class MathStyle {
    /** `\text{...}` â€” upright, rendered as ordinary text with spaces preserved. */
    TEXT,

    /** `\mathbf{...}` â€” bold upright. */
    BOLD,

    /** `\boldsymbol{...}` â€” bold italic, used for bold greek letters and bold variables. */
    BOLD_ITALIC,

    /** `\mathit{...}` â€” italic (default for letters, but useful to force it). */
    ITALIC,

    /** `\mathrm{...}` â€” upright roman. */
    ROMAN,

    /** `\mathbb{...}` â€” double-struck (via Unicode mapping where available). */
    DOUBLE_STRUCK,

    /** `\mathcal{...}` â€” calligraphic (best-effort Unicode mapping). */
    CALLIGRAPHIC,
}
