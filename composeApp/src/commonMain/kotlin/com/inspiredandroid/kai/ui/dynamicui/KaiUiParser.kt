 ...
 *   1. **Block extraction** â€” the markdown parser locates `kai-ui` fences and hands the raw
 *      body to [parseUiBlockBody].
 *   2. **Syntax repair** â€” fix broken key syntax, trim mismatched braces, close truncated JSON
 *      so `parseToJsonElement` can succeed.
 *   3. **Direct build** â€” walk the resulting [kotlinx.serialization.json.JsonElement] tree via
...
object KaiUiParser {
    ...
    private fun sanitizeJson(raw: String): String {
        ...
        // Unclosed JSON â€” trim trailing incomplete content and then close open structures.
    ...
    private fun trimTrailingIncomplete(json: String, inString: Boolean): String {
        ...
        while (s.isNotEmpty()) {
            ...
            if (last != '"') break
            // Possible orphaned key â€” find its opening quote.
