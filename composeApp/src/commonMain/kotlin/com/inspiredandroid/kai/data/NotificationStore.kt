...
/**
 * Persistence for notifications captured by [com.kai.custom.notifications.KaiNotificationListenerService].
 *
 * Two collections:
 * - **Pending queue** â€” capped FIFO that fills as the listener fires and gets snapshotted
 *   into the heartbeat prompt, then drained. Mirrors [SmsStore].
 * - **Store** â€” broader rolling history backing [com.kai.custom.notifications.NotificationReader],
 *   bounded by per-app cap and age cap.
 *
 * Per-app gating is handled by the system Notification Access "Apps" picker â€” if the
