...
// The heartbeat prompt is a single shape â€” always sent as a user message.
 ...
 *
 * @param customOrDefaultPrompt leading free text â€” custom user prompt or [HeartbeatManager.DEFAULT_HEARTBEAT_PROMPT]
...
internal fun buildHeartbeatPrompt(
    ...
    if (heartbeatAdditions.isNotEmpty()) {
        append("\n## Heartbeat Additions\n")
        append("Standing instructions the user asked to run on every heartbeat. Address each in your response alongside the main self-check â€” if all are satisfied and nothing else needs attention, respond with your acknowledgement rather than HEARTBEAT_OK (the additions are the attention).\n")
