...
// Pure builders for the chat system prompt. Every input is passed explicitly â€” no DI,
// no suspend, no resource loading, no Clock â€” so tests can call `buildChatSystemPrompt`
 ...
 * [DataRepository.getActiveSystemPrompt]'s signature â€” callers pick the variant based on
 * whether they're dispatching to a remote or on-device service.
 */
enum class SystemPromptVariant {
    /** Full chat prompt for remote services â€” every section available. */
    CHAT_REMOTE,

    /**
     * Trimmed prompt for on-device LiteRT plain chat â€” only sections a 2-4B Gemma can
     ...
     * modes (the latter is also hidden from the UI for on-device services â€” see
     * `ChatScreen.kt`).
     */
    CHAT_LOCAL,
}
...
internal data class ChatPromptRuntimeContext(
    ...
    /** UTC ISO 8601 with `Z` suffix â€” kept so the model can double-check the offset. */
...
/**
 * Shared shape for rendering a connected email account into a prompt section â€” used by
 ...
 * when building the `CHAT_LOCAL` variant. Memories are appended in order â€” general â†’
 * preferences â†’ learnings â†’ errors â€” and entries that would push the combined size
 ...
 * No header â€” a single sentence doesn't earn a `##` section.
 */
 ...
 * (not in the soul string) so it survives user customization of the soul â€” the same
...
internal const val DEFAULT_TOOL_USE_SECTION =
    "## Tool Use\n" +
        ...
        "Summarize noisy output and state any uncertainty â€” don't dump raw logs."
 ...
 * `CHAT_REMOTE` variant â€” scheduling tools aren't in the local allowlist, and the
 ...
 * should map to `null`). All inputs are passed explicitly â€” memory lists are pre-split
...
internal fun buildChatSystemPrompt(
    ...
    // Language preference â€” adapt to the user's language
    ...
    append("## Language\nYour preferred language is $langName. Adapt to the user's language in both text and voice. Speak in the same language the user uses â€” if they write in Indonesian, respond and speak in Indonesian; if they write in English, respond and speak in English. When using speak_text, always pick the voice that matches the language you're speaking. Default voice: $defaultVoice.")
    ...
    // Semantic memory section â€” most relevant memories from vector/FTS search appear
    // first so the model sees what's directly relevant before the full category dump.
    if (relevantMemories.isNotEmpty()) {
        if (isNotEmpty()) append("\n\n")
        append("## Relevant Memories\n")
        append("The following memories are semantically related to the current conversation:\n")
    ...
    // remote models â€” the local model should be able to reference them. A char-count budget
    ...
    // Automation guidance + Email Accounts + Scheduled Tasks stay remote-only â€” the
...
private fun StringBuilder.appendMemoryCategorySection(
    ...
    if (included == 0) {
        // Not even the first entry fit â€” don't emit the header alone.
...
private fun StringBuilder.appendEmailAccountsSection(accounts: List<EmailAccountSummary>) {
    append("\n\n## Email Accounts\n")
    append("The user has these email accounts connected. Use them via the existing email tools â€” ")
...
private fun StringBuilder.appendDynamicUiSection() {
    ...
    append("Proactively use them whenever you need input from the user â€” don't just ask in plain text if a form, selector, or buttons would be more natural. ")
    ...
    append("You can mix kai-ui blocks with regular markdown text naturally â€” use markdown for explanations and kai-ui for interactive elements.\n\n")
    append(KAI_UI_COMPONENT_CATALOG)
    append("Layout tips:\n")
    append("- Put buttons INSIDE cards, directly below related content â€” never group all buttons separately at the bottom\n")
    append("- Use rows for groups of buttons or chips â€” rows wrap automatically, so any number of items is fine\n")
...
private fun StringBuilder.appendInteractiveUiSection() {
    ...
    append("You are in full-screen interactive UI mode. The user ONLY sees rendered kai-ui components â€” they cannot see markdown, plain text, or anything outside a kai-ui fence.\n")
    ...
    append("Limitations â€” respect these strictly:\n")
    ...
    append("- Never use indeterminate progress (progress without a value) or text like \"Loading...\", \"Fetching...\", \"Verifying...\" as if something will happen later â€” nothing will.\n")
    append("- Each screen is independent. Only conversation history carries state between screens â€” there is no client-side state persistence, no session storage, no variables that survive across responses.\n")
...
/**
 * Pre-computed kai-ui component catalog text â€” shared between [appendDynamicUiSection]
...
private val KAI_UI_COMPONENT_CATALOG: String = buildString {
    ...
    append("- text: {\"type\":\"text\",\"value\":\"...\",\"style\":\"headline|title|body|caption\",\"bold\":true,\"color\":\"primary|secondary|error\"} â€” do NOT use markdown formatting (**, *, #, etc.) in text values; use the bold/italic/style properties instead\n")
    ...
    append("- chip_group: {\"type\":\"chip_group\",\"id\":\"...\",\"chips\":[{\"label\":\"Tag\",\"value\":\"tag\"}],\"selection\":\"single|multi|none\"} â€” selection mode: \"single\" (default, one at a time), \"multi\" (any number), or \"none\" (display-only tags, no interaction, id not needed). For \"single\" and \"multi\" a button must collectFrom the chip_group id to send the selection.\n")
    append("- list: {\"type\":\"list\",\"items\":[...],\"ordered\":false} â€” bullet (or numbered) list; the app renders bullets/numbers automatically, so do NOT include bullet characters (â€¢, -, *) or numbering in item text\n")
    append("- table: {\"type\":\"table\",\"headers\":[\"Col1\",\"Col2\"],\"rows\":[[\"a\",\"b\"]]} â€” columns share equal width; keep to 3-4 columns max on mobile, use short cell values\n")
    append("- icon: {\"type\":\"icon\",\"name\":\"home|settings|search|add|delete|edit|check|check_circle|close|arrow_back|arrow_forward|star|favorite|share|info|warning|person|group|mail|phone|calendar|location|refresh|menu|more|send|notifications|trending_up|trending_down|trending_flat|thumb_up|thumb_down|visibility|lock|shopping_cart|play|pause|stop|download|upload|cloud|attachment|link|code|terminal|build|bug|lightbulb|science|school|work|account_circle|language|translate|dark_mode|light_mode|bolt|rocket|money|credit_card|receipt|inventory|category|dashboard|analytics|chart|pie_chart|show_chart|timer|alarm|task|bookmark|flag|tag|pin|copy|paste|cut|undo|redo|filter|sort|swap|sync|wifi|bluetooth|battery|speed|shield|verified|health|fitness|food|coffee|airplane|hotel|car|earth|map|compass|pet|leaf|water|weather|party|trophy|medal|premium\",\"size\":24,\"color\":\"primary|secondary|error\"} â€” you can also use any emoji as the name (e.g. \"name\":\"âš”ï¸\" or \"name\":\"ðŸ—ºï¸\"); prefer emojis when they better convey the meaning than the generic Material icons\n")
    append("- code: {\"type\":\"code\",\"code\":\"...\",\"language\":\"kotlin\"} â€” a copy-to-clipboard icon is rendered automatically; do NOT add your own copy button next to it.\n")
    ...
    append("- quote: {\"type\":\"quote\",\"text\":\"...\",\"source\":\"Author Name\"} â€” blockquote with accent border\n")
    append("- badge: {\"type\":\"badge\",\"value\":\"3\",\"color\":\"primary|secondary|error\"} â€” small colored pill for counts or status\n")
    append("- stat: {\"type\":\"stat\",\"value\":\"\$1,234\",\"label\":\"Revenue\",\"description\":\"12% increase\"} â€” large metric display\n")
    append("- avatar: {\"type\":\"avatar\",\"name\":\"John Doe\",\"imageUrl\":\"https://...\",\"size\":40} â€” circular image or initials (24-80dp)\n\n")
    append("Actions (on buttons, countdown expiry):\n")
    append("- callback: {\"type\":\"callback\",\"event\":\"event_name\",\"data\":{\"key\":\"val\"},\"collectFrom\":[\"input_id1\",\"input_id2\"]} â€” collects input values and sends them back as a user message (e.g. \"Pressed: event_name\" or \"Responded with: key: value\"). You then reply with text or more UI. Use callbacks for: collecting choices, submitting forms, navigating between steps, confirming actions. Do NOT create callback buttons that imply operations you cannot perform â€” callbacks only send a message, they do not trigger system actions like printing, file export, or downloads.\n")
    append("- toggle: {\"type\":\"toggle\",\"targetId\":\"element_id\"} â€” shows/hides an element locally\n")
    append("- open_url: {\"type\":\"open_url\",\"url\":\"https://...\"}\n")
    append("- copy_to_clipboard: {\"type\":\"button\",\"action\":{\"type\":\"copy_to_clipboard\",\"text\":\"...\"}} â€” renders as a clipboard icon button; omit the button label. Offer next to copyable content like snippets, commands, or tokens.\n\n")
