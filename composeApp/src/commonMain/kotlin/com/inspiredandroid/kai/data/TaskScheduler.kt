...
class TaskScheduler(
    ...
    private companion object {
        ...
        /** Per-task execution log size â€” surfaced in the task details sheet. */
        const val MAX_TASK_LOG_ENTRIES = 10

        /**
         * Cap the notification body â€” Android's collapsed text cuts off around ~60
     ...
     * is cancelled â€” as long as the OS keeps the process alive (which on Android means
     ...
     * On Android this mirrors `ProcessLifecycleOwner` â€” set true on the first Activity
    ...
    /**
     * Starts the scheduler loop on the internal long-lived scope. Idempotent â€” repeated
    ...
    fun start() {
        ...
        activeJob = schedulerScope.launch {
            while (isActive) {
                ...
                // SMS polling â€” FOSS-only (gated on `isSmsSupported`, which is true only
    ...
    private suspend fun runHeartbeat() {
        ...
        try {
            ...
            if (response.isNotBlank() && "HEARTBEAT_OK" !in response) {
                ...
                // Strip markdown + kai-ui fences before sending to the tray â€”
            ...
            // Only clear the snapshot we actually showed to the AI â€” messages
    ...
    /**
     * User-pressed manual heartbeat (Settings â†’ Agent â†’ Heartbeat refresh icon). Bypasses
    ...
    private fun truncateForNotification(text: String): String {
        ...
        // Only prefer the word boundary if it's close to the cap; otherwise hard-cut â€”
        ...
        return window.substring(0, cut).trimEnd().trimEnd(',', ';', ':') + "â€¦"
    }
    ...
    /**
     * Format an exception for the task log. Plain `e.message` collapses too much detail â€”
    ...
    private suspend fun handleTaskFailure(task: ScheduledTask, error: String? = null) {
        ...
        if (task.cron != null) {
            // Cron task failed â€” advance to the next scheduled time instead of retrying every cycle
            ...
            // One-time task â€” apply exponential backoff
    ...
    private suspend fun handleTaskCompletion(task: ScheduledTask) {
        ...
        if (task.cron != null) {
            // Recurring task â€” compute next execution time
            val nextExecution = try {
                CronExpression(task.cron).nextAfter(now)
            } catch (e: Exception) {
                // Cron computation failed â€” leave pending for retry
            ...
            if (nextExecution != null) {
                ...
                // No valid future time â€” mark completed
                taskStore!!.updateTask(
                    task.copy(
                        status = TaskStatus.COMPLETED,
