...
object CommonTools {
    ...
    val openUrlTool = object : Tool {
        override val schema = ToolSchema(
            name = "open_url",
            description = "Open a URL in the user's browser or default app. This ONLY opens the link for the user to view â€” you will NOT receive the page content back. Do not use this to fetch or read information from URLs. Use this when the user asks to open or visit a link.",
            parameters = mapOf(
    ...
    // Tool IDs gated by master toggles in Settings â†’ Agent (isMemoryEnabled / isSchedulingEnabled /
    // isEmailEnabled / isSmsEnabled / isSmsSendEnabled). They stay in `commonToolDefinitions` so the
    // chat UI can resolve their display names, but the Tools tab filters them out â€” toggling them
