    ...
    fun scheduleTaskTool(taskStore: TaskStore) = object : Tool {
        override val schema = ToolSchema(
            ...
            parameters = mapOf(
                ...
                "execute_at" to ParameterSchema(type = "string", description = "ISO 8601 datetime for a one-off run. Either offset-qualified (e.g. '2025-03-15T09:00:00+02:00' or '2025-03-15T07:00:00Z') â€” interpreted as that exact instant â€” OR naive (e.g. '2025-03-15T09:00:00') â€” interpreted in the user's local timezone shown in `## Context`. Prefer offset-qualified to avoid ambiguity. Must be in the future.", required = false),
        ...
        override suspend fun execute(args: Map<String, Any>): Any {
            ...
            if (triggerCount > 1) {
                return mapOf("success" to false, "error" to "execute_at, cron, and on_heartbeat are mutually exclusive â€” pick one")
            }
            ...
            val scheduledAtEpochMs = if (executeAt != null) {
                ...
                if (parsed < nowMs - PAST_INSTANT_SLACK_MS) {
                    return mapOf(
                        "success" to false,
                        "error" to "execute_at ($executeAt) is in the past â€” check the Local time in Context and retry (use an offset-qualified value like 2025-03-15T09:00:00+02:00 to avoid UTC/local ambiguity)",
                    )
                }
                parsed
            } else {
