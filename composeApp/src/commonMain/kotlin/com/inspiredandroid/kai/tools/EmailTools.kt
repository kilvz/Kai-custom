...
object EmailTools {
    ...
    fun readEmailTool(emailStore: EmailStore) = object : Tool {
        override val schema = ToolSchema(
            ...
            parameters = mapOf(
                ...
                "mark_read" to ParameterSchema(type = "boolean", description = "Whether to mark the email as seen on the server (default false â€” reading is non-destructive). Set true only when the user has actually dealt with the email and wants it out of their unread list.", required = false),
            ),
        )
    ...
    fun searchEmailTool(emailStore: EmailStore) = object : Tool {
        override val schema = ToolSchema(
            name = "search_email",
            description = "Search emails by sender, subject, or date across the whole inbox (read and unread). Prefer this over check_email whenever the user mentions a specific sender, subject, or time range â€” e.g. \"unsubscribe from X\", \"the email from Alice last week\". Returns matching email summaries with an `is_read` flag.",
            parameters = mapOf(
