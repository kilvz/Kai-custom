    ...
    fun sendSmsTool(smsDraftStore: SmsDraftStore, smsSender: SmsSender) = object : Tool {
        override val schema = ToolSchema(
            ...
            description = "Draft an outgoing SMS. The draft is staged in a banner at the top of the chat " +
                "so the user must explicitly tap Send before anything is actually sent. You cannot bypass this â€” " +
        ...
        override suspend fun execute(args: Map<String, Any>): Any {
            ...
            return mapOf(
                ...
                "to" to to,
                "body" to body,
                "message" to "Draft created. Waiting for the user to tap Send in the review banner â€” nothing has been sent yet.",
            )
        }
    }
    ...
    fun replySmsTool(smsDraftStore: SmsDraftStore, smsReader: SmsReader, smsSender: SmsSender) = object : Tool {
        override val schema = ToolSchema(
            ...
            description = "Draft a reply to a received SMS. Looks up the original by id to pick the sender, then stages a draft " +
                "in the review banner â€” the user must tap Send to actually send.",
            parameters = mapOf(
        ...
        override suspend fun execute(args: Map<String, Any>): Any {
            ...
            return mapOf(
                ...
                "message" to "Reply draft created. Waiting for the user to tap Send in the review banner â€” nothing has been sent yet.",
            )
        }
    }
