...
#include <unistd.h>      /* read(2), write(2), close(2), unlink(2), */
#include <fcntl.h>       /* open(2), O_RDONLY, O_WRONLY|O_CREAT|O_EXCL */
#include <sys/stat.h>    /* fstat(2), S_ISREG, struct stat */
...
#undef POKE_WORD
	...
	case PR_linkat: {
		...
		if ((int) syscall_result != -EPERM && (int) syscall_result != -EACCES)
			goto end;

		if (syscall_number == PR_link) {
			status = read_path(tracee, oldpath, peek_reg(tracee, MODIFIED, SYSARG_1));
			if (status < 0) break;
			status = read_path(tracee, newpath, peek_reg(tracee, MODIFIED, SYSARG_2));
			if (status < 0) break;
		}
		else {
			status = read_path(tracee, oldpath, peek_reg(tracee, MODIFIED, SYSARG_2));
			if (status < 0) break;
			status = read_path(tracee, newpath, peek_reg(tracee, MODIFIED, SYSARG_4));
			if (status < 0) break;
		}

		source_fd = open(oldpath, O_RDONLY);
		if (source_fd < 0) { status = -errno; break; }

		if (fstat(source_fd, &st) < 0) { status = -errno; close(source_fd); break; }
		if (!S_ISREG(st.st_mode)) { close(source_fd); goto end; }

		unlink(newpath);

		target_fd = open(newpath, O_WRONLY|O_CREAT|O_EXCL, st.st_mode & 0777);
		if (target_fd < 0) { saved_errno = errno; close(source_fd); status = -saved_errno; break; }

		{
			char copy_buf[4096];
			ssize_t nread;
			while ((nread = read(source_fd, copy_buf, sizeof(copy_buf))) > 0) {
				ssize_t pos = 0;
				while (pos < nread) {
					ssize_t nwrite = write(target_fd, copy_buf + pos, nread - pos);
					if (nwrite <= 0) { saved_errno = errno; close(source_fd); close(target_fd); status = -saved_errno; break; }
					pos += nwrite;
				}
				if (status < 0) break;
			}
			if (nread < 0 && status >= 0) { saved_errno = errno; close(source_fd); close(target_fd); status = -saved_errno; break; }
		}
